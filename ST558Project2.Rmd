...
```{r}
#Changing to factors and renaming
...
A linear regression model fits explanatory variable(s) to a response variable. The coefficients for the variables are given based on the line that would provide the smallest number when calculating the squared difference between the observed values to the expected values along the line. If there is a singular explanatory variable to predict the explanatory variable, it would be considered a simple linear regression. If there are more than one variable, interaction terms, and/or polynomial terms then it is considered mulitple linear regression. 

```{r}
#SLR with temperature as the explanatory variable
model1 <- lm(cnt ~ atemp, data = Day.Train)
summary(model1)

#Model with season indicator variables
model2 <- lm(cnt ~ season, data = Day.Train)
summary(model2)

#MLR models
model3 <- lm(cnt ~ atemp*season, data = Day.Train)
summary(model3)

model4 <- lm(cnt ~ season*workingday, data = Day.Train)
summary(model4)
```
