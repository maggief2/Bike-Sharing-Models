...
Our other variables of interest are qualitative, so we will create contingency tables to see how many observations fall under each category.
...
Day.Train$weathersit <- factor(Day.Train$weathersit, c(1, 2), c("Good", "Bad"))

#Contingency tables for each categorical variable
kable(table(Day.Train$season), caption = "Frequency by Season")
kable(table(Day.Train$workday), caption = "Frequency by Workday")
...
#Workday
ggplot(Day.Train, aes(workday, cnt)) + 
  geom_boxplot() +
  geom_jitter(aes(color = workday)) +
