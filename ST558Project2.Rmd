...
In this report, we will be analyze the day.csv bike sharing data set. We will start by summarizing the data to get an idea of the data set. Then we will try to predict the number of users using predictive models. The data set contains the count of bike share users per day over 731 days along with the weather and climate data associated with each day. The response variable that we are trying to predict is the `cnt` variable which a count of all biking sharing users in the given day. Our goal is to predict `cnt` based on what season it is, `season`, whether it is a workday or not, `workday`, how the temperature feels that day, `atemp`, the wind speed that day, `windspeed`, the humidity, `hum`, and more. We will predict the count of bike shares with 4 models: 2 different linear regression models, one Random Forest model, and one Boosted Tree model. After we fit the 4 models, we compare the models on a test set and declare the winner. The winner will be the model that predicts `cnt` the best; this will be the model with the smallest Test MSE.   
...
ggplot(data = Day.Train, mapping = aes(x=cnt)) +
  ...
  geom_density(kernal = "gaussian", size = 2, color = 'red') +
